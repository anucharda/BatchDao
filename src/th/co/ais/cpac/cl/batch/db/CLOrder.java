...
public class CLOrder {
	...
	protected class GetOrderTreatementInfoByMobileAndAction
		...
		protected StringBuilder createSqlProcess() {
			...
			sql.append(" a.ORDER_ID,BA_NO,MOBILE_NO,TREATMENT_ID,BATCH_ID ").append(ConstantsDB.END_LINE);
			sql.append(" FROM CL_ORDER_TREATMENT a ").append(ConstantsDB.END_LINE);
			sql.append(" INNER JOIN dbo.CL_ORDER b ").append(ConstantsDB.END_LINE);
			sql.append(" ON a.ORDER_ID=b.ORDER_ID ").append(ConstantsDB.END_LINE);
			sql.append(" WHERE MOBILE_NO = ('").append(mobileNo).append("') ").append(ConstantsDB.END_LINE);
			sql.append(" and ACTION_STATUS = (").append(actStatus).append(")").append(ConstantsDB.END_LINE);
	...
	protected class UpdateOrderStatus extends DBTemplatesUpdate<ExecuteResponse, UtilityLogger, DBConnectionPools> {
		...
		protected StringBuilder createSqlProcess() {
			StringBuilder sql = new StringBuilder();
			sql.append("UPDATE dbo.CL_ORDER ").append(ConstantsDB.END_LINE);
			...
			sql.append(",SFF_ORDER_NO = '").append(sffOrderNo).append("'").append(ConstantsDB.END_LINE);
			if (failReason != null) {
			...
			sql.append(" AND ACTION_STATUS  = ").append(ConstantsDB.actInprogressStatus).append(ConstantsDB.END_LINE);
			return sql;
		}
	...
	protected class GetOrderTreatementInfoByTreatment
		...
		protected StringBuilder createSqlProcess() {
			StringBuilder sql = new StringBuilder();
			sql.append(" SELECT").append(ConstantsDB.END_LINE);
			sql.append(" TREATMENT_ID,MOBILE_NO,BA_NO,BATCH_ID,ACTION_STATUS ").append(ConstantsDB.END_LINE);
			sql.append(" FROM CL_ORDER_TREATMENT a ").append(ConstantsDB.END_LINE);
			sql.append(" INNER JOIN dbo.CL_ORDER b ").append(ConstantsDB.END_LINE);
			sql.append(" ON a.ORDER_ID=b.ORDER_ID ").append(ConstantsDB.END_LINE);
