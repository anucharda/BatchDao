...
public class CLBlacklist {
	...
	protected class GetBlacklistAction
		...
		protected StringBuilder createSqlProcess() {
			...
			sql.append(" C.BLACKLIST_ID,ROW_ID ").append(ConstantsDB.END_LINE);
			sql.append(" FROM CL_BLACKLIST C, CPDB..SFF_BLACKLIST S ").append(ConstantsDB.END_LINE);
			sql.append(" WHERE C.BLACKLIST_OPTION = 1 ").append(ConstantsDB.END_LINE);
			sql.append(" AND C.BLACKLIST_TYPE = S.BLACKLIST_TYPE ").append(ConstantsDB.END_LINE);
			sql.append(" AND C.BLACKLIST_SUBTYPE = S.BLACKLIST_SUBTYPE ").append(ConstantsDB.END_LINE);
			sql.append(" AND C.BLACKLIST_SOURCE = S.BLACKLIST_SOURCE ").append(ConstantsDB.END_LINE);
			...
			sql.append(" AND ACTION_STATUS = (").append(actStatus).append(")")
					.append(ConstantsDB.END_LINE);
			sql.append(" AND BATCH_ID = (").append(batchID).append(")").append(ConstantsDB.END_LINE);
			sql.append(" AND S.BILLING_ACCNT_ID = (SELECT B.SFF_ACCOUNT_ID FROM CL_BA_INFO B WHERE B.BA_NO = '")
					.append(baNo).append("') ").append(ConstantsDB.END_LINE);
			sql.append(" AND C.BA_NO = '").append(baNo).append("' ").append(ConstantsDB.END_LINE);
			sql.append(
					" AND convert(Varchar(10),C.BLACKLIST_REQUEST_DATE,112)+ str_replace(convert(Varchar(15),BLACKLIST_REQUEST_DATE,108),':',null) = '")
					.append(blacklistDtm).append("' ").append(ConstantsDB.END_LINE);
	...
	protected class GetDeBlacklistAction
		...
		protected StringBuilder createSqlProcess() {
			...
			sql.append(" C.BLACKLIST_ID,ROW_ID ").append(ConstantsDB.END_LINE);
			sql.append(" FROM CL_BLACKLIST C, CPDB..SFF_BLACKLIST S ").append(ConstantsDB.END_LINE);
			sql.append(" WHERE C.BLACKLIST_OPTION = 2 ").append(ConstantsDB.END_LINE);
			sql.append(" AND C.BLACKLIST_TYPE = S.BLACKLIST_TYPE ").append(ConstantsDB.END_LINE);
			sql.append(" AND C.BLACKLIST_SUBTYPE = S.BLACKLIST_SUBTYPE ").append(ConstantsDB.END_LINE);
			sql.append(" AND C.BLACKLIST_SOURCE = S.BLACKLIST_SOURCE ").append(ConstantsDB.END_LINE);
			...
			sql.append(
					" AND convert(Varchar(10),C.BLACKLIST_REQUEST_DATE,112)+ str_replace(convert(Varchar(15),BLACKLIST_REQUEST_DATE,108),':',null) = '")
					.append(blacklistEndDtm).append("' ").append(ConstantsDB.END_LINE);
			sql.append(" AND C.BLACKLIST_TYPE = '").append(blacklistType).append("' ")
					.append(ConstantsDB.END_LINE);
			sql.append(" AND C.BLACKLIST_SUBTYPE = '").append(blacklistSubType).append("' ")
					.append(ConstantsDB.END_LINE);
	...
	protected class UpdateBlacklistStatus extends DBTemplatesUpdate<ExecuteResponse, UtilityLogger, DBConnectionPools> {
		...
		protected StringBuilder createSqlProcess() {
			StringBuilder sql = new StringBuilder();
			sql.append("UPDATE dbo.CL_BLACKLIST ").append(ConstantsDB.END_LINE);
			sql.append("SET LAST_UPD= getdate() , LAST_UPD_BY='").append(updateBy).append("'")
					.append(ConstantsDB.END_LINE);
			sql.append(",ACTION_STATUS = ").append(actionStatus).append(ConstantsDB.END_LINE);
			sql.append(", ACTION_STATUS_DTM = getdate() ").append(ConstantsDB.END_LINE);
			sql.append(",SFF_BLACKLIST_ID  = '").append(sffRowID).append("'").append(ConstantsDB.END_LINE);
			sql.append(" WHERE BLACKLIST_ID  = ").append(blacklistId).append(ConstantsDB.END_LINE);
			return sql;
		}
		...
		protected ExecuteResponse execute(int actionStatus, String sffRowID, BigDecimal blacklistId,String updateBy) {
			...
			return executeUpdate(ConstantsDB.getDBConnectionPools(logger), true); // case
																				// adjust
																				// false;
		}
	}
	...
	protected class UpdateBlacklistStatusByBatchID extends DBTemplatesUpdate<ExecuteResponse, UtilityLogger, DBConnectionPools> {
		...
		protected StringBuilder createSqlProcess() {
			...
			sql.append("SET LAST_UPD= getdate() , LAST_UPD_BY='").append(updateBy).append("'")
					.append(ConstantsDB.END_LINE);
			sql.append(",ACTION_STATUS = ").append(resultStatus).append(ConstantsDB.END_LINE);
			sql.append(", ACTION_STATUS_DTM = getdate() ").append(ConstantsDB.END_LINE);		
			sql.append(",ACTION_REMARK = '").append(actRemark).append("'").append(ConstantsDB.END_LINE);
			sql.append(" WHERE ACTION_STATUS   = ").append(actionStatus).append(ConstantsDB.END_LINE);
