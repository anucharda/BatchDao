...
public class CLBaInfo {
	...
	protected class UpdateBaInfoAction extends DBTemplatesUpdate<ExecuteResponse, UtilityLogger, DBConnectionPools> {
		...
		protected StringBuilder createSqlProcess() {
			...
			sql.append("UPDATE dbo.CL_BA_INFO B ").append(ConstantsDB.END_LINE);
			sql.append("SET LAST_UPD= getdate() , LAST_UPD_BY='").append(username).append("'").append(ConstantsDB.END_LINE);
			sql.append(",WRITEOFF_BOO  = 'Y'").append(ConstantsDB.END_LINE);
			sql.append(",WRITEOFF_DATE  = ").append(writeOffDtm).append(ConstantsDB.END_LINE);
			sql.append(",WRITEOFF_TYPE  = (SELECT A.WRITEOFF_TYPE_CODE FROM CL_WRITEOFF_TYPE A WHERE A.WRITEOFF_TYPE_ID = ").append(writeOffTypeID).append(")").append(ConstantsDB.END_LINE);
