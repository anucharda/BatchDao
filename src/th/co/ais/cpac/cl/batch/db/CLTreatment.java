...
public class CLTreatment {
	...
	protected class UpdateWriteOffFailAction extends DBTemplatesUpdate<ExecuteResponse, UtilityLogger, DBConnectionPools> {
		...
		protected StringBuilder createSqlProcess() {
			...
			sql.append("JOIN CL_WRITEOFF_TREATMENT WT ON T.TREATMENT_ID = WT.TREATMENT_ID ").append(ConstantsDB.END_LINE);
			sql.append("JOIN CL_WRITEOFF W ON WT.WRITEOFF_ID = W.WRITEOFF_ID ").append(ConstantsDB.END_LINE);
			sql.append("WHERE T.ACTION_STATUS = 3 ").append(ConstantsDB.END_LINE);
			sql.append("AND W.BATCH_ID = ").append(batchID).append(ConstantsDB.END_LINE);
			return sql;
		}
	...
	protected class UpdateTreatmentByBlacklistIDAction extends DBTemplatesUpdate<ExecuteResponse, UtilityLogger, DBConnectionPools> {
		...
		protected StringBuilder createSqlProcess() {
			...
			sql.append(", ACTION_STATUS_DTM  = getdate() ").append(ConstantsDB.END_LINE);
			sql.append(" FROM CL_TREATMENT T ").append(ConstantsDB.END_LINE);
			sql.append("JOIN CL_BLACKLIST_TREATMENT BT  on T.TREATMENT_ID=BT.TREATMENT_ID AND BT.BLACKLIST_ID = (").append(blacklistID).append(") ").append(ConstantsDB.END_LINE);
			sql.append("JOIN CL_BLACKLIST B on B.BLACKLIST_ID = BT.BLACKLIST_ID ").append(ConstantsDB.END_LINE);
			sql.append("JOIN CL_ACTION A ON T.ACTION_ID = A.ACTION_ID AND A.ACTION_MODE IN (10,11) ").append(ConstantsDB.END_LINE);
	...
	protected class UpdateTreatmentByBatchIDAction extends DBTemplatesUpdate<ExecuteResponse, UtilityLogger, DBConnectionPools> {
		...
		protected StringBuilder createSqlProcess() {
			...
			sql.append("UPDATE dbo.CL_TREATMENT ").append(ConstantsDB.END_LINE);
			sql.append("SET LAST_UPD= getdate() , LAST_UPD_BY='").append(username).append("'").append(ConstantsDB.END_LINE);
			sql.append(",ACTION_STATUS = ").append(actResultStatus).append(ConstantsDB.END_LINE);
			sql.append(", ACTION_STATUS_DTM  = getdate() ").append(ConstantsDB.END_LINE);
			sql.append(", ACTION_REMARK   ='").append(actRemark).append("'").append(ConstantsDB.END_LINE);
			sql.append(" FROM dbo.CL_TREATMENT  T ").append(ConstantsDB.END_LINE);			
			sql.append(" JOIN CL_BLACKLIST_TREATMENT BT on T.TREATMENT_ID=BT.TREATMENT_ID ").append(ConstantsDB.END_LINE);
			sql.append(" JOIN CL_BLACKLIST  B  on BT.BLACKLIST_ID=B.BLACKLIST_ID ").append(ConstantsDB.END_LINE);
			sql.append(" JOIN CL_ACTION A ON T.ACTION_ID = A.ACTION_ID AND A.ACTION_MODE IN (10,11) ").append(ConstantsDB.END_LINE);
			sql.append(" WHERE T.ACTION_STATUS   = ").append(actStatus).append(ConstantsDB.END_LINE);
			sql.append(" AND B.BATCH_ID= ").append(batchID).append(ConstantsDB.END_LINE);
			return sql;
		}
